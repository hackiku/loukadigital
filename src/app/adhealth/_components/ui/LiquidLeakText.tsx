...
export function LiquidLeakText({ className = "" }: LiquidLeakTextProps) {
	...
	return (
		...
		>
			...
			<span className="absolute inset-0 bg-neutral-500/10 backdrop-blur-sm z-0" style={{ borderRadius }} />

			{/* Liquid layer - smooth gradient fill from bottom */}
			<span className="absolute inset-0 overflow-hidden z-10" style={{ borderRadius }}>
				<span
					className="absolute inset-x-0 bottom-0"
					style={{
						height: "65%",
						background:
							"linear-gradient(180deg, rgba(239, 68, 68, 0.4) 0%, rgba(249, 115, 22, 0.5) 50%, rgba(249, 115, 22, 0.6) 100%)",
				...
				>
					...
					>
						...
						<path fill={`url(#waveGradient-${uniqueId})`} d="M0,25 Q300,10 600,25 T1200,25 L1200,50 L0,50 Z">
							<animate
								attributeName="d"
								dur="2s"
                ...
                "
					...
					>
						...
						<path fill={`url(#waveGradient2-${uniqueId})`} d="M0,28 Q300,43 600,28 T1200,28 L1200,50 L0,50 Z">
							<animate
								attributeName="d"
								dur="2.3s"
								repeatCount="indefinite"
